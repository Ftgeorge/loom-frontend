            ...
            <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
                    ...
                    <Card className="py-5 px-4 mb-4 rounded-[24px] border border-gray-50 shadow-sm" style={{ shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 8, elevation: 2 }}>
                        <View className="flex-row justify-between h-[120px] items-end">
                            {earnings.weeklyData.map((d, i) => (
                                <Animated.View entering={StretchInY.delay(300 + i * 100)} key={d.day} className="items-center flex-1">
                ...
                <Animated.View entering={FadeInUp.delay(500)}>
                    ...
                    {earnings.transactions.map((tx, index) => (
                        <Animated.View entering={FadeInUp.delay(700 + index * 100).springify()} key={tx.id} className="flex-row items-center py-4 gap-4 border-b border-gray-100">
                            <View
